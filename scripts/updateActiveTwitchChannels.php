...
class UpdateAllPointRankings extends Script {
    ...
    public static function getTrends($guildDetails, $rankDetails, $rankSystem, $dungeonId) {
        ...
        $oldRankDetails = self::getGuildRankingsForDungeon($guildDetails->_guildId, $dungeonId);
        $dungeonDetails = CommonDataContainer::$dungeonArray[$dungeonId];
        $server         = $guildDetails->_server;
        $region         = $guildDetails->_region;
        $country        = $guildDetails->_country;
        ...
        if ( !empty($oldRankDetails) ) {
            ...
            */
            $identifier = strtolower($rankSystem);
            $currentWorldRank   = $oldRankDetails[$identifier . '_world_rank'];
            $currentServerRank  = $oldRankDetails[$identifier . '_server_rank'];
            $currentRegionRank  = $oldRankDetails[$identifier . '_region_rank'];
            $currentCountryRank = $oldRankDetails[$identifier . '_country_rank'];

            $currentWorldTrend   = ( !empty($oldRankDetails[$identifier . '_world_trend']) ? $oldRankDetails[$identifier . '_world_trend'] : '--' );
            $currentServerTrend  = ( !empty($oldRankDetails[$identifier . '_server_trend']) ? $oldRankDetails[$identifier . '_server_trend'] : '--' );
            $currentRegionTrend  = ( !empty($oldRankDetails[$identifier . '_region_trend']) ? $oldRankDetails[$identifier . '_region_trend'] : '--' );
            $currentCountryTrend = ( !empty($oldRankDetails[$identifier . '_country_trend']) ? $oldRankDetails[$identifier . '_country_trend'] : '--' );
        ...
        foreach ( self::$_encounterKillTimeArray as $encounterId => $guildArray ) {
            ...
            foreach ( $guildArray as $guildId => $strtotime ) {
                ...
                if ( !isset(self::$_dungeonPointArray[$dungeonId]['QP'][$guildId]) ) {
                    ...
                    if ( $dungeonDetails->_finalEncounterId == $encounterId && (!isset($dungeonFinalEncounterCheck[$dungeonId]) || $dungeonFinalEncounterCheck[$dungeonId] == true) ) {
    ...
    public static function getGuildRankingsForDungeon($guildId, $dungeonId) {
        $rankDetails = array();
        ...
        $rankDetails = $query->fetch(PDO::FETCH_ASSOC);
