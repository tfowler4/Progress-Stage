    ...
    public function setClears() {
        ...
        foreach( CommonDataContainer::$guildArray as $guildId => $guildDetails ) {
            if ( isset($guildDetails->_encounterDetails->$encounterId) ) {                
                ...
                $encounterKillOrderedArr[$guildId] = $encounterCompleteTime;
        ...
        switch ( $this->_numOfEncounterKills ) {
            case 0:
                ...
                break;
            case 1:
                reset($encounterKillOrderedArr);
                $firstGuild  = CommonDataContainer::$guildArray[key($encounterKillOrderedArr)]->_nameLink;
                $recentGuild = CommonDataContainer::$guildArray[key($encounterKillOrderedArr)]->_nameLink;
                break;
            default:
                reset($encounterKillOrderedArr);
                $firstGuild = CommonDataContainer::$guildArray[key($encounterKillOrderedArr)]->_nameLink;
                arsort($encounterKillOrderedArr);
                reset($encounterKillOrderedArr);
                $recentGuild = CommonDataContainer::$guildArray[key($encounterKillOrderedArr)]->_nameLink;
                break;
        }
